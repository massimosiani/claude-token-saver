...
Adapted from https://github.com/drona23/claude-token-efficient, then audited against
...
1. **The savings are behavioral, not typographic.** Scope creep, self-verification
...
- A rule marked **(Claude 4.6+)** is tuned to a documented behavior of that model
  generation and carries its reason inline. It is safe on other agents but earns its
  tokens only on Claude; delete those lines if you target a different model.
...
- Keep output short by being selective about what goes in, not by compressing how it is written. Drop details that do not change what the reader does next.
- Write what you do include in complete sentences with terms spelled out. Fragments, invented abbreviations and arrow chains cost the reader a reread, which erases the saving.
...
- Do not add a verification pass on top of your normal work, and do not spawn an agent to check your own output. (Claude 4.6+: these models verify themselves, so a re-verification instruction buys duplicate work and no extra correctness.)
- Delegate to a subagent only when the payoff clearly exceeds the overhead. Each one re-establishes context, re-explores, reports back, and then you re-read the report; work you could finish in a handful of tool calls is cheaper done directly. (Claude 4.6+: these models reach for subagents readily, and each one multiplies cost.)
...
- Correct an earlier statement when the error changes the reader's code, conclusions or decisions. State the correction plainly and carry on, without apologies or a tally of past mistakes. (Claude 4.6+: these models narrate self-corrections at length, which reads as thrash.)
- Match the length of written deliverables to what the task needs. Cover the substance without padding documents with filler sections, redundant summaries or boilerplate. (Claude 4.6+: files these models write to disk run longer than on earlier models.)
